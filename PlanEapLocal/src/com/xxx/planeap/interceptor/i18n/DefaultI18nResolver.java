...
/**
 * 默认的资源文件解析器，该类读取
 * <code>org.springframework.context.i18n.LocaleContextHolder</code>
 * 中保存的Local信息解析资源文件
 * 
 * @author WangXuzheng
 * @see org.springframework.context.i18n.LocaleContextHolder
 */
public class DefaultI18nResolver extends MessageSourceSupport implements
		I18nResolver {
	...
	private static final Logger LOG = Logger
			.getLogger(DefaultI18nResolver.class);
	 ...
	 * @return
	 */
	private String findDefaultText(String textName, Locale locale) {
		List<String> localList = DEFAULT_RESOURCE_BUNDLES;
		for (String bundleName : localList) {
			ResourceBundle bundle = findResourceBundle(bundleName, locale);
			if (bundle != null) {
				try {
					return bundle.getString(textName);
	 ...
	 */
	protected ResourceBundle findResourceBundle(String bundleName,
			Locale locale) {
		String key = createMissesKey(bundleName, locale);
		ResourceBundle bundle = BUNDLE_MAP.get(key);
		if (bundle == null) {
			bundle = ResourceBundle.getBundle(bundleName, locale, Thread
	...
	public String getMessage(String code, Object[] args, String defaultMessage) {
		return org.apache.commons.lang.StringUtils.defaultIfBlank(getMessage(
